...
let ``Should create erc20 transfer call`` () =
    let data =
        transferCall
            web3
            "0x42775d50b7Db4768f32d0267b399DE8ED7e56700"
            "0x95ADDFfF52B727E0d2317a2f1f255350f743813E"
            10
...
let ``Should create transaction hash`` () =
    asyncResult {
        let p: UnwrapParameters =
            { TokenContract = "0x42775d50b7Db4768f32d0267b399DE8ED7e56700"
              Destination = "0x95ADDFfF52B727E0d2317a2f1f255350f743813E"
              Amount = 10I
              OperationId = "ooLfc6nEYiHH7jUfGHLahCuPS7YkQRyNNt3Thamoy24664EFtDK" }


        let! hash = transactionHash web3 "0x6e3d2fF2C4727B9E7F50D9604D7D661de2Ac2c46" p
...
let ``Should sign`` () =
    asyncResult {
        ...
        let! r = Signer.memorySigner(key).Sign(hash)
